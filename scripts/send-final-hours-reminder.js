...
import { finalHoursReminderEmail } from '../api/_lib/email-templates.js';
import { sendEmail } from '../api/_lib/mailer.js';

const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const campaignType = 'submission_final_2h30_20260909';
const requiredConfirmation = 'SEND-FINAL-HOURS-REMINDER';
...
if (Date.now() >= new Date(config.submissionDeadlineAt).getTime()) throw new Error('The submission deadline has passed.');
...
console.log(`Pending reminder recipients: ${pending.length}`);
...
if (!shouldSend) {
  ...
  for (const participant of pending) {
    ...
    try {
      const token = await issueParticipantToken(participant.id, 'final-hours-reminder');
      const result = await sendEmail(participant.email, finalHoursReminderEmail(participant, token), `final-hours-20260909/${participant.id}`);
  ...
  console.log(`Final-hours reminder complete: ${sentCount} sent, ${skippedCount} newly completed and skipped, ${failedCount} failed.`);
